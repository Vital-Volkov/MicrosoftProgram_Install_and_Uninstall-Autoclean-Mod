param($CreateRestorePoint, $PackageGUID, $ScriptName)

. .\Win8Library.ps1
Import-LocalizedData -BindingVariable Strings_WinMini8Strings -FileName Strings 
if(IsMiniPack)
{
    write-diagprogress -activity $Strings_WinMini8Strings.ProgressText_Resolving
}

function Pre-Resolve
{
    $isClient = IsClientOS
    $RestorepointCreated = DetectRestorePoint
    if($($CreateRestorePoint -ieq "true") -And $isClient -And !$RestorepointCreated)
    {
        if(IsMiniPack)
        {
            write-diagprogress -activity $Strings_WinMini8Strings.ProgressText_CreatingRestorePoint
        }
        $restorePointDesc = "Installed Microsoft Solution - " + $PackageGUID
        CreateRestorepoint($restorePointDesc)
        MarkRestorePoint
        if(IsMiniPack)
        {
            write-diagprogress -activity $Strings_WinMini8Strings.ProgressText_Resolving
...
{
    $argstring = ParseArgs $Arguments
    $expression = ".\$ScriptName " + $argstring
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU7T2DmBWBJGlk5WUUGUhh/n4s
# 8D+gghWCMIIEwzCCA6ugAwIBAgITMwAAAI1AOzwvrcZ7uAAAAAAAjTANBgkqhkiG
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFKkT
# z2uyG1G/jfhX/XV64WWrinYBME4GCisGAQQBgjcCAQwxQDA+oByAGgBXAGkAbgA4
# AFIAQwAuAFIAUwAuAHAAcwAxoR6AHGh0dHA6Ly9zdXBwb3J0Lm1pY3Jvc29mdC5j
# b20wDQYJKoZIhvcNAQEBBQAEggEADE2F1wQcGi1+M4kYw9JHsedqT995G6ONFbj5
# 3SbxppTtw3BIrrybnNdOIG+ydvJproKhAEMGQFmc/cunBus+9eHSax904wpBMuyJ
# fzdGS3Vdyc/Ki2VQnx3m9eG3j6fCLeLMlANfYI41m3YaJ4oU0gkrkjxI+fp5miwz
# HDaJ6DkJxSsJ1GRSJSHY6V6FHVznkeoRWOrWo1UozdRxdkN/ERW4o9WsXAs3Awgr
# WU3cPDN/WY92Q7qaPqrRqUUHj5Ea/a6xeCGjjZ1raUldZRs6W5HpvROxouoSruTQ
# 34gGAhvFrqy7VfWgE+IXsu2YT9U5zEYQouYJYLmzD1r//HG8ZKGCAigwggIkBgkq
...
# hvcNAQkEMRYEFNdSjPmRl5Nz8LuncL0K5WsazP+cMA0GCSqGSIb3DQEBBQUABIIB
# AEewTWbpV8INk1zFZj6mUioR7pHo3mAN/ztH9dRqfNh5WRfNhTUUyUTaQoFuITA5
# MQ7cW2LmTkaDUcAX9E5vG+ksfcgnbE3vjOzvCpjG3OAS/HZsebzpUK7f5SgMR2y+
# bBuJZAnCazd0FljuOyOmBKdxRMtcxlkAIGkSUqpBtGSYHm4bfRW9wqFUX34FA+NT
# BFVdPDlMgAdwgC40u+aXmKSvSdE/MraGzKWHvFH0qWT/hvVB8+K068y3EG+Trf+M
# 7/Lfb0moSY8tJldhF8tOKPVtXbA3CVTPfgb/BEIUHR60L8cFBJehHrjPkLzT198x
# siwGGI9y/zki8RZ2gE94s9g=
